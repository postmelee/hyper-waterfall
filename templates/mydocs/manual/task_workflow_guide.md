...
Hyper-Waterfall 방법론 자체를 새 저장소에 설치하거나 기존 적용 저장소를 새 version으로 업데이트하는 작업은 일반 기능 task와 다른 배포 프로토콜을 따른다.
...
- 후속 Skill: `framework-install`, `framework-update`

`framework-install`과 `framework-update` Skill은 M020 후속 작업에서 추가된다. Skill이 아직 없을 때는 `docs/agent-entrypoint.md`와 migration guide를 먼저 읽고, 기존 사용자 수정 파일을 덮어쓰기 전에 작업지시자 승인을 받아야 한다.
...
후속 `framework-install`, `framework-update` Skill이 추가되면 설치·업데이트 절차에도 같은 호출 표시 원칙을 적용한다.
