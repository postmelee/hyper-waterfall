...
신규 적용은 strict manifest 기준으로 수행한다. 허용 대상은 manifest `files[]`의 target, `.hyper-waterfall/version.json`, manifest가 정의한 symlink이며, manifest 밖 파일이나 디렉터리는 생성하거나 수정하지 않는다. `docs/**`, `src/**`, `examples/**`, `schemas/**`, `package.json`, `tsconfig.json` 같은 경로는 신규 적용 중 금지 예시일 뿐 대상 저장소의 일반 문서 구조를 정의하지 않는다.
