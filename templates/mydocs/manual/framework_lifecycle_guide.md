...
CLI가 `update`나 `doctor` 결과를 만들더라도 이 결과는 파일 변경 전 판단 보고다. CLI 출력은 현재 version, 목표 release/tag, migration guide, manifest diff, 자동 적용 가능, 수동 확인 필요, conflict, 보류, 검증, 승인 요청을 포함해야 하며, 승인 전에는 파일을 실제 대상 저장소에 적용하지 않는다.
