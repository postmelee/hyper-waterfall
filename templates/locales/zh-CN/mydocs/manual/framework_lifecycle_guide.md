...
即使 CLI 输出 `init`、`update` 或 `doctor` 结果，该结果也是变更前判断报告。CLI 输出必须包含 selected locale、`.hyper-waterfall/version.json` 的 `locale` record plan、current version、current locale、requested locale 或 locale switch request、target release/tag、target release locale support、migration guide、manifest diff、locale manifest diff、automatically applicable items、items requiring manual review、conflicts、deferred items、verification 和 approval request。批准前不要把文件应用到目标仓库。
