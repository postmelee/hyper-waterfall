...
| 4. 단계 종료 | 단계 보고서 검토, 다음 단계 승인 | 단계 보고서 작성, 단계 묶음 커밋 | `task-stage-report` + `todo` |
| 5. 최종 보고 | 최종 보고서 검토, PR 생성 승인 | 최종 보고서 작성, Open PR 생성 | `task-final-report` + `todo` |
| 6. 리뷰 & merge | 리뷰, merge 결정 | PR 본문 보강, 검증 결과 반영 | (수동) |
| 7. merge 후 정리 | 정리 승인 | 이슈 close, 브랜치 정리, 오늘할일 마감 | `pr-merge-cleanup` + `todo` |
| 별도. 외부 PR 검토 | 외부 PR 검토 요청 | 검토 문서 작성, 검증, 리뷰 응답 | `external-pr-review` |

> 이슈 진행 추적은 GitHub의 **linked PR** 자동 cross-reference와 **라벨/마일스톤**에 위임합니다 (kubernetes·vscode·rust 등 빅테크 OSS 표준). 상태 알림 댓글은 사용하지 않고, 댓글은 토론·블로커·결정 기록 용도로만 둡니다.
