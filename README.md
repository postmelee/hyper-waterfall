...
기존 적용 저장소 업데이트를 위한 GitHub Release/tag, manifest, migration guide 기준도 별도 문서로 정리하고 있습니다. 본 저장소 자체가 자기 자신에 적용한 dogfooding 첫 사례입니다 (Issue #1, PR #2).
...
기존 적용 저장소 업데이트는 GitHub Release/tag와 manifest를 기준으로 수행합니다. 상세 프로토콜은 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md)와 [`docs/migrations/`](docs/migrations/)에 두고, README에는 추후 확정되는 업데이트 프롬프트나 CLI 명령만 안내합니다.
...
| `task-final-report` | 모든 Stage가 끝나고 PR을 게시하기 직전 | `final_report.md` 기반 최종 보고서, 오늘할일 완료 처리, 검증 결과·근거를 구조화한 Open PR |
