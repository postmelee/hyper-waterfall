...
**(2) 운영 규칙·SKILL·매뉴얼·템플릿 분리**: `AGENTS.md`에는 매 턴 시스템 프롬프트로 적재될 정책·제약·인덱스만 두고, 절차 상세는 [`mydocs/manual/`](templates/mydocs/manual/)의 5개 매뉴얼(문서 구조, 타스크 진행, Git, PR, 충돌 규칙), [`mydocs/_templates/`](templates/mydocs/_templates/)의 문서 출력 형식, [`mydocs/skills/`](templates/mydocs/skills/)의 7개 SKILL로 분리했습니다.
...
- **의도 전달 명확성**: 단계별 SKILL이 절차의 어느 시점에 호출되는지 명시하고, 중앙 템플릿이 산출물의 모양을 고정해 AI가 추론으로 절차나 출력 형식을 재구성하지 않습니다.
...
문서 템플릿의 진실 원천은 `templates/mydocs/_templates/`, SKILL의 진실 원천은 `templates/mydocs/skills/`입니다. 적용 저장소에서는 `.agents/skills`와 `.claude/skills` 심볼릭 링크가 같은 Skill 본문을 가리킵니다.
