...
“대충 시키면 AI가 알아서 다 만들어준다”는 방식도 아닙니다.

대신 AI 페어 프로그래밍에 필요한 소프트웨어 개발 방법론을 저장소에 설치 가능한 템플릿으로 만든 것입니다. 작업을 GitHub Issue, 수행계획서, 구현계획서, Stage, 단계별 완료보고서, 최종 보고서, PR로 쪼개고, 각 단계마다 작업지시자의 명시 승인을 받도록 강제합니다.
...
AI는 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md)를 먼저 읽고, 신규 적용은 [`docs/lifecycle/adoption.md`](docs/lifecycle/adoption.md), 기존 업데이트는 [`docs/lifecycle/update.md`](docs/lifecycle/update.md)를 기준으로 변경 후보를 보고합니다. 파일 변경 전에는 작업지시자 승인을 요청합니다. npm CLI는 dry-run 판단을 보조하는 채널이며, canonical 기준은 GitHub Release/tag, `templates/manifest.json`, migration guide입니다.
...
신규 적용은 Hyper-Waterfall 운영 파일과 작업 기억 구조만 설치합니다. `docs/`, `specs/`, `site/`, `website/`, `adr/` 같은 대상 프로젝트의 공식 문서 루트는 자동 생성하지 않으며, 제품/사용자/API/아키텍처/로드맵 문서가 필요하면 별도 Issue의 수행계획서에서 문서 위치 판단을 승인받아 생성합니다.

도입 후에는 AI가 알아서 Hyper-Waterfall 방식을 지키며 작업을 진행합니다. 처음 시작하는 사용자는 AI에게 `"이거 구현해줘"`와 같은 자연어 명령을 내리기만 하면 됩니다.

기존 적용 저장소 업데이트는 GitHub Release/tag와 manifest를 기준으로 수행합니다. AI는 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md)를 진입점으로 삼고, [`docs/lifecycle/update.md`](docs/lifecycle/update.md)의 기존 업데이트 판단 결과 형식에 따라 현재 version, 목표 release/tag, migration guide, manifest diff, Hyper-Waterfall 버전 업데이트 PR 후보를 먼저 보고합니다. 승인된 업데이트 후보를 PR로 전환할 때는 [`docs/lifecycle/update_pr.md`](docs/lifecycle/update_pr.md)를 따릅니다. npm CLI는 같은 판단을 실행하기 쉽게 하는 편의 실행 채널이며, canonical 기준인 GitHub Release/tag, `templates/manifest.json`, migration guide를 대체하지 않습니다. CLI 출력만으로 파일을 자동 적용하지 않고, 승인된 범위만 일반 task 흐름으로 전환합니다.
...
`hyper-waterfall@0.2.0`은 npm registry에 publish되어 있으며, 아래 `npx` 명령으로 lifecycle 판단을 실행할 수 있습니다. publish 전후 검증 결과는 [`docs/releases/v0.2.0-npm-publish.md`](docs/releases/v0.2.0-npm-publish.md)에서 관리합니다.
