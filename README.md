...
9. [배포와 업데이트](#배포와-업데이트)
10. [도입 후 작업 흐름](#도입-후-작업-흐름)
11. [적용 후 대상 저장소 구조](#적용-후-대상-저장소-구조)
12. [살아있는 예시 — 직접 따라가보기](#살아있는-예시--직접-따라가보기)
13. [설계 원칙](#설계-원칙)
14. [프롬프트 가이드 준수](#프롬프트-가이드-준수)
...
### 1. 프롬프트 한 줄로 적용, GitHub Release 기준으로 업데이트 — 모듈화 + placeholder 치환
...
다만 배포의 기준은 프롬프트 문장이 아니라 GitHub Release/tag와 [`templates/manifest.json`](templates/manifest.json)입니다. 적용 저장소는 `.hyper-waterfall/version.json`으로 현재 Hyper-Waterfall version을 기록하고, 기존 적용 저장소는 [`docs/migrations/`](docs/migrations/)의 migration guide를 기준으로 업데이트 차이와 충돌 가능성을 검토합니다. 본 저장소 자체가 자기 자신에 적용한 dogfooding 첫 사례입니다 (Issue #1, PR #2).
...
프롬프트는 설치나 업데이트를 시작하는 인터페이스입니다. 실제 기준은 GitHub Release/tag, `templates/manifest.json`, `.hyper-waterfall/version.json`, `docs/migrations/`입니다.

- **GitHub Release/tag**: Hyper-Waterfall의 canonical 배포 단위입니다. npm, Homebrew, Docker, plugin 같은 채널은 후속 실행 채널이 될 수 있지만 release 기준을 대체하지 않습니다.
- **`templates/manifest.json`**: release에 포함되는 파일, 적용 대상 경로, `overwrite`/`merge`/`manual`/`preserve`/`symlink` update policy, checksum 상태를 정의합니다.
- **`.hyper-waterfall/version.json`**: 적용 저장소가 현재 사용하는 Hyper-Waterfall version과 release tag를 기록하는 상태 파일입니다. 작업 문서가 아니므로 `mydocs/`에 두지 않습니다.
- **`docs/migrations/`**: 기존 적용 저장소가 새 version으로 이동할 때 추가 파일, 수정 파일, 수동 확인, 충돌 가능성, 검증 기준을 확인하는 migration guide입니다.

새 저장소에 처음 적용할 때는 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md)의 신규 적용 절차를 따릅니다. 이미 적용한 저장소를 업데이트할 때는 현재 version과 목표 release의 manifest 차이를 확인하고, migration guide의 수동 확인 항목을 update PR에 남겨야 합니다. 후속 M020 작업에서 `framework-install`과 `framework-update` Skill은 이 기준을 실행 절차로 감싸게 됩니다.
...
배포 manifest의 진실 원천은 `templates/manifest.json`입니다. 적용 저장소는 `.hyper-waterfall/version.json`으로 현재 version을 기록하고, 기존 적용 저장소 업데이트는 `docs/migrations/`의 migration guide를 함께 확인합니다.
