...
| 제공되는 것 | 의미 |
|---|---|
| 한 줄 적용 | AI가 진입 문서부터 읽고 대상 저장소에 필요한 파일과 placeholder 치환 후보를 보고합니다. |
| 승인 게이트 | 소스 수정, 단계 전환, 최종 보고, PR 생성 전에 사람이 방향을 확인합니다. |
| 작업 기억 외부화 | Issue, 계획서, Stage 보고서, 최종 보고서, PR, commit log에 작업 맥락이 남습니다. |
| 공식 프롬프팅 가이드 정합 | 목표, 맥락, 출력 형식, 검증 기준, 멈춤 조건을 저장소 구조와 템플릿으로 고정합니다. |
| Multi-agent 운영 | Codex, Claude Code 등 여러 AI 에이전트가 같은 규칙과 SKILL을 읽고 작업합니다. |
