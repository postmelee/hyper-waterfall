...
已有应用仓库的更新基于 GitHub Release/tag 和 manifest 执行。AI 以 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md) 为入口，按照 [`docs/lifecycle/update.md`](docs/lifecycle/update.md) 的既有更新判断结果格式，先报告当前 version、当前 locale、requested locale 或 switch request、目标 release/tag、目标 release locale support、migration guide、manifest diff、locale manifest diff、Hyper-Waterfall version update PR 候选。
