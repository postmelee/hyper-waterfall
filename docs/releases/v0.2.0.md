...
| `docs/migrations/v0.1.0-to-v0.2.0.md` | 대상 version, canonical 배포 단위, manifest 위치, checksum 상태, 보류 항목이 release 전 기준을 설명한다. Stage 3에서 manifest checksum 상태 표현과 재대조한다. | Stage 3 대조 예정 |
...
Task #26 Stage 2 기준으로 version/tag 필드 불일치나 canonical 기준 충돌은 발견되지 않았다. 실제 Git tag와 GitHub Release는 아직 생성하지 않았으므로 `release.status`는 `planned`로 유지한다.
...
| 항목 | 명령 | 현재 기록 |
|---|---|---|
| 원격 tag 동기화 | `git fetch --tags origin` | Stage 3에서 재검증 |
| Git tag 목록 | `git tag --list --sort=version:refname` | Stage 3에서 `v0.2.0` tag 미생성 여부 재검증 |
| GitHub Release 목록 | `gh release list --repo postmelee/hyper-waterfall --limit 20` | Stage 3에서 `v0.2.0` release 미생성 여부 재검증 |
| manifest parse | `ruby -rjson -e 'JSON.parse(File.read("templates/manifest.json"))'` | Stage 2 통과 |
| release 기준 grep | `rg -n '0.2.0|v0.2.0|plannedTag|baselineTag|GitHub Release|manifest|migration|pending-release|ready|checksum|승인' ...` | Stage 2 통과 |
| diff 공백 검사 | `git diff --check` | Stage 2 통과 |

Task #26 Stage 2 기준으로 실제 tag와 GitHub Release는 아직 생성되지 않았다. 다음 작업은 Stage 3에서 관련 문서의 checksum 표현을 재대조한 뒤, 작업지시자가 명시 승인한 경우에만 `git tag v0.2.0`, `git push origin v0.2.0`, `gh release create v0.2.0` 실행 여부를 결정하는 것이다.
