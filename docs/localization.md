...
| GitHub Issue/PR template | 대상 | Issue/PR 작성자가 읽는 플랫폼 template이므로 locale pack 포함 대상 후보로 둔다. |
| `templates/mydocs/_templates/*` | 대상 | 수행계획서, 구현계획서, Stage 보고서, 최종 보고서 등 생성 문서의 섹션명과 작성 언어를 결정한다. |
| `templates/mydocs/manual/*` | 대상 후보 | 에이전트 운영 manual이므로 locale pack 구조와 유지 비용을 #67에서 확정한다. |
| `templates/mydocs/skills/*` | 대상 후보 | Skill은 절차 실행 규격이므로 번역 시 의미 보존 검증이 필수다. 구조는 #67에서 확정한다. |
...
각 `files[]` entry의 `localization.enabled`가 `true`이면 해당 entry는 선택 locale의 `sourcePattern`을 우선 적용 후보로 사용한다. `.gitkeep`과 Skill discovery symlink처럼 자연어가 아닌 구조 파일은 `localization.enabled: false`로 둔다.
