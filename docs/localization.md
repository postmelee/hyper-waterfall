...
- 신규 적용에서 승인된 locale은 `.hyper-waterfall/version.json`의 top-level `locale` 필드에 기록한다.
- 기존 적용 저장소 update는 `.hyper-waterfall/version.json`의 `locale` 값을 기본으로 보존한다. 기존 적용 기록에 `locale`이 없으면 `unknown`으로 보고하고 작업지시자에게 보존 또는 전환 판단을 승인받는다.
- update 중 locale 전환은 부수 효과로 자동 처리하지 않고, 작업지시자의 명시 요청과 별도 승인 항목으로 분리한다.
