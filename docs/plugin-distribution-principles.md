...
| hook/guardrail | plugin hook feature flag, hooks path, adapter 후보만 확인. hook 구현은 별도 승인 대상 | plugin hook 비활성 기본값, 활성화 UX, block/warn/report-only 안내 확인 | `hooks/hooks.json` 후보와 adapter 후보만 확인. hook 구현은 별도 승인 대상 | hook merge 동작과 opt-in 보조 guardrail 안내 확인 |
