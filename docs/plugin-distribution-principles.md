...
다음 항목은 Stage 4에서 연계 문서와 최종 checklist로 정리한다.

- `docs/distribution-channels.md`에서 이 문서를 Codex/Claude plugin 공통 원칙으로 연결
- README 또는 `docs/agent-entrypoint.md`에 필요한 경우 짧은 cross-reference 추가
- #37-#40 packaging 검증 checklist 확정
- core Skill 또는 manual 본문 변경이 필요하지 않았음을 최종 확인
