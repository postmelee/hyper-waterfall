...
따라서 migration guide는 단순 changelog가 아니라 lifecycle 판단 결과, Hyper-Waterfall 버전 업데이트 PR 리뷰, CLI `update`/`doctor` 출력의 근거 문서로 작성한다.

## CLI 출력 기준

CLI는 migration guide를 대체하지 않는다. `init`, `update`, `doctor`는 release manifest, `.hyper-waterfall/version.json`, migration guide를 읽어 다음 판단 결과를 출력하고, 실제 파일 변경은 작업지시자 승인 후 일반 task 흐름에서 수행한다.
