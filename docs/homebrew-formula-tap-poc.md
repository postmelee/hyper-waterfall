...
아래는 설계 기준을 보여주는 초안이다. Task #34 local smoke에서는 이 초안에 실제 npm tarball SHA256을 채운 formula 후보를 `/private/tmp` local tap에서 검증했다. 이 문서만으로 public tap에 push하지 않는다.
...
Task #34에서 확인한 npm tarball SHA256:

```text
34dc90ca4b9cefa3f13034711e6bffc3f3c184360c44ab4924e00e26163e0cc7
```
...
Task #34에서는 registry tarball을 파일로 내려받은 뒤 `shasum -a 256`으로 위 SHA256을 확인했다.
...
아래 명령은 public tap 배포 전 검증 경로의 후보다. Task #34에서는 public tap 대신 `/private/tmp` local tap으로 같은 핵심 smoke를 실행했다.
...
Task #35 결과 기준으로 public tap technical smoke는 통과했다. 사용자-facing 설치 명령은 `brew install postmelee/tap/hyper-waterfall`이다. 아무 tap도 지정하지 않는 `brew install hyper-waterfall` 첫 설치 경로는 Homebrew core 등재가 필요하다. [#46](https://github.com/postmelee/hyper-waterfall/issues/46) 검토 결과, 현재는 notability와 non-author usage, 전체 지원 플랫폼 검증, stronger formula test 근거가 부족하므로 core 제출을 보류하고 public tap 경로를 유지한다.
...
- Homebrew core 제출 재검토: notability, non-author usage, x86_64 macOS/Linux 검증, stronger formula test 근거 확보 후 별도 이슈로 진행
...
Task #35에서 `postmelee/homebrew-tap` public repository와 `postmelee/tap/hyper-waterfall` 설치 경로가 공개됐고, public tap smoke가 통과했다. Homebrew는 계속 CLI 설치 wrapper이며 canonical 기준은 GitHub Release/tag, `templates/manifest.json`, migration guide다. `brew install hyper-waterfall` 단독 경로는 Homebrew core 등재가 필요하지만, #46 검토 기준 현재는 core 제출을 보류하고 public tap 경로를 유지한다.
