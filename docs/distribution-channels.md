...
|---|---|---|---|
| GitHub Release/tag | `v0.3.0` 공개 완료 | 현재: `https://github.com/postmelee/hyper-waterfall/releases/tag/v0.3.0` | release마다 manifest, migration guide, release notes 정합성 유지 |
| npm CLI | `hyper-waterfall@0.3.0` 공개 완료 | 현재: `npx hyper-waterfall@0.3.0 --help` | post-publish smoke 통과. release마다 publish 전후 검증 필요 |
| Homebrew public tap | `postmelee/tap/hyper-waterfall` 공개 완료, formula는 아직 `0.2.0` tarball 기준 | `brew install postmelee/tap/hyper-waterfall` | `0.3.0` npm tarball URL과 checksum으로 formula 갱신 필요 |
...
M050의 `v0.3.0` release는 다국어 적용 진입점을 배포하는 단계다. release 상태와 post-publish 검증 결과는 [`docs/releases/v0.3.0.md`](releases/v0.3.0.md)를 기준으로 관리한다.

| 항목 | release 기준 |
|---|---|
| package metadata | `package.json` version `0.3.0`, npm tarball `files`에 `README.ko.md`, `README.zh-CN.md` 포함 |
| manifest metadata | `templates/manifest.json`의 `frameworkVersion: 0.3.0`, `release.plannedTag: v0.3.0`, `baselineTag: v0.2.0`, `release.status: released` |
...
| release execution | GitHub Release/tag 생성과 npm publish 완료. Homebrew tap 갱신은 별도 승인 항목 |
...
| M050 release 완료 | `v0.3.0` GitHub Release/tag, npm `hyper-waterfall@0.3.0`, 다국어 README, localized lifecycle docs, migration guide, npm pack 구성 |
| 이번 task에서 상태 정리 | `templates/manifest.json` release status, migration guide 보류 항목, distribution 문서의 채널별 현재 상태 |
| 후속 승인 필요 | Homebrew `0.3.0` formula 갱신, Homebrew core 제출, Docker read-only image, Codex official public 배포, Claude Official directory 제출, Claude release asset 게시, release automation, root/directory checksum 산식 확정 |
...
- `v0.2.0`과 `v0.3.0` GitHub Release/tag는 공개 완료 상태다.
- 계속 P0로 유지한다.
...
`hyper-waterfall@0.2.0`과 `hyper-waterfall@0.3.0`은 npm registry에 publish되어 있다. `v0.3.0` publish 전후 검증 결과는 `docs/releases/v0.3.0.md`를 기준으로 확인한다.
...
1. P0: `v0.3.0` GitHub Release/tag, manifest, migration guide, npm CLI publish 상태를 유지하고 release마다 같은 검증을 반복한다.
2. P1: Homebrew public tap은 `postmelee/tap/hyper-waterfall` 경로로 유지하고, `0.3.0` formula 갱신을 후속 작업으로 처리한다. core 제출은 #46 보류 판단에 따라 조건 충족 후 재검토한다.
