# task_m020_9_impl.md - 프로토콜 기반 update PR 워크플로우 정착 구현계획서
...
| Stage | 제목 | 주요 산출 | 검증 |
|---|---|---|---|
| 1 | update PR workflow 경계와 명명 규칙 정의 | `docs/agent-entrypoint.md`, `git_workflow_guide.md`, `task_workflow_guide.md` | 생성 조건, 브랜치, 커밋, PR 제목, task/release/update PR 경계 grep |
...
| 3 | README/migration guide 연결과 CLI 기준 정리 | `README.md`, `docs/migrations/*`, 관련 Manual | README 짧은 안내, update PR/CLI 연결, 폐기된 예정 구현 표현 부재 확인 |

## Stage 1 - update PR workflow 경계와 명명 규칙 정의
...
- update PR이 #8의 기존 업데이트 판단 결과 승인 후 생성되는 후속 작업임을 명시한다.
- 일반 task PR, release PR, update PR의 목적과 흐름을 구분한다.
- update PR 브랜치명, 커밋 메시지, PR 제목 규칙을 정한다.
- update PR도 사람이 리뷰할 수 있는 일반 PR로 남기되, 입력은 lifecycle 판단 결과와 migration guide임을 명시한다.
- 브랜치 규칙은 후속 CLI가 반복 실행할 수 있도록 충돌 가능성이 낮은 이름으로 정의한다.

### 검증

```bash
grep -nE 'update PR|브랜치|커밋|PR 제목|release PR|task PR' docs/agent-entrypoint.md templates/mydocs/manual/*.md
...
```text
Task #9 Stage 1: update PR 경계와 명명 규칙 정의
```
...
- #8의 기존 업데이트 판단 결과 필드를 update PR 본문 항목으로 옮기는 매핑을 정의한다.
...
- 별도 update PR 전용 템플릿이 필요한지 검토하되, 필요하지 않다면 Manual/agent entrypoint의 작성 규칙으로 충분하다는 판단을 문서화한다.
...
```text
Task #9 Stage 2: update PR 본문 구조 정의
```
...
- README에는 기존 적용 저장소 업데이트가 판단 결과 승인 후 update PR로 진행된다는 짧은 안내만 둔다.
- migration guide가 update PR 본문과 CLI `update`/`doctor` 출력의 근거 문서임을 보강한다.
...
```bash
grep -nE 'update PR|migration|version|manifest diff|CLI|doctor|update' README.md docs/migrations/*.md templates/mydocs/manual/*.md
...
```text
Task #9 Stage 3: README와 migration guide update PR 연결
```
...
- Stage 2는 Stage 1에서 update PR의 경계와 명명 규칙이 확정된 뒤 진행한다.
- Stage 3은 Stage 2에서 PR 본문 구조와 변경 분류 기준이 확정된 뒤 README와 migration guide를 연결한다.
- #10 CLI는 Stage 2~3의 update PR 본문 구조와 출력 필드를 `update`/`doctor` 명령 출력 기준으로 참고한다.
...
- **README 과다 설명**: README에는 짧은 update PR 흐름만 남기고 상세 규칙은 agent entrypoint, migration guide, Manual에 둔다.
- **후속 CLI 범위 침범**: 이번 task는 CLI가 사용할 출력 필드와 검증 기준까지만 정의하고 구현은 #10에 남긴다.
- **이전 접근 표현 재유입**: PR 본문과 문서에서 특정 폐기된 구현 방식처럼 읽히는 표현을 피하고, lifecycle/update PR workflow의 현재 의사결정만 설명한다.

## 승인 요청 사항

- Stage 1~3 분할, 산출물, 검증 명령, 커밋 메시지에 동의?
- update PR 본문은 기존 PR 템플릿 재사용을 우선 검토하고, 필요할 때만 최소 보강하는 방향에 동의?
