# task_m020_9.md - 프로토콜 기반 update PR 워크플로우 정착 수행계획서
...
#8에서 정리한 framework lifecycle 판단 결과를 실제 리뷰 가능한 update PR로 전환하는 표준 워크플로우를 정의한다.
...
기존 PR 템플릿은 이미 요약, 변경 내역, 검증, 검증 한계, 남은 리스크를 담는다. 따라서 이번 task는 새 플랫폼 템플릿을 먼저 늘리기보다, update PR이 기존 PR 템플릿 안에서 어떤 항목을 반드시 채워야 하는지와 어떤 경우에 별도 섹션이 필요한지를 판단한다.

## 범위

### 포함

- update PR 생성 조건과 단계 정의
- update PR 브랜치명, 커밋 메시지, PR 제목 규칙 정의
...
- 기존 PR 템플릿을 그대로 사용할지, update PR 전용 섹션 또는 별도 템플릿이 필요한지 검토
...
- update PR은 #8의 lifecycle 판단 결과가 승인된 뒤 별도 이슈와 브랜치에서 수행한다.
...
- 별도 update PR 템플릿은 필요성이 검증될 때만 제안하고, 이번 task에서는 템플릿 중복보다 문서 규칙과 예시를 우선한다.
...
- **Stage 1 — update PR workflow 경계와 명명 규칙 정의**
  - update PR이 언제 생성되는지, 일반 task PR/release PR과 어떻게 구분되는지, 브랜치·커밋·PR 제목 규칙을 정리한다.
...
- **Stage 3 — README/migration guide 연결과 후속 CLI 기준 정리**
  - README에는 짧은 update PR 흐름만 남긴다.
  - migration guide가 update PR 리뷰 근거 문서임을 보강한다.
...
- update PR의 생성 조건, 브랜치명, 커밋 메시지, PR 제목 규칙이 문서에서 확인된다.
- 사용자 수정 파일은 자동 적용 대상이 아니라 수동 확인 또는 conflict로 분류된다.
- migration guide와 manifest diff가 update PR 리뷰 근거로 연결된다.
...
- **PR 템플릿 중복**: update PR 전용 템플릿을 섣불리 만들면 플랫폼 템플릿이 분산된다. 기존 PR 템플릿 재사용을 우선하고, 필요한 차이는 Manual과 작성 규칙으로 둔다.
...
- update PR을 기존 PR 템플릿 재사용 중심으로 설계하고, 필요한 차이는 Manual과 작성 규칙에 두는 방향에 동의?
- 변경 분류를 `자동 적용 가능`, `수동 확인 필요`, `conflict`, `보류` 기준으로 고정하는 방향에 동의?
- #10 CLI 구현 전 이번 task에서는 update PR workflow 문서화와 본문 구조 정의까지만 다루는 범위에 동의?
